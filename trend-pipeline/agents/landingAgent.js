...
const THEMES = {
  'AI tools':          { primary: '#7c3aed', bg: '#0f0f1e', surface: '#1a1a38', accent: '#a855f7', ring: '#7c3aed40' },
  'health & wellness': { primary: '#16a34a', bg: '#0a1a0a', surface: '#122812', accent: '#22c55e', ring: '#16a34a40' },
  'personal finance':  { primary: '#1d4ed8', bg: '#0a1020', surface: '#101e38', accent: '#60a5fa', ring: '#1d4ed840' },
  'productivity':      { primary: '#4f46e5', bg: '#0e0e1c', surface: '#18184a', accent: '#818cf8', ring: '#4f46e540' },
  'relationships':     { primary: '#be185d', bg: '#180a12', surface: '#261222', accent: '#f472b6', ring: '#be185d40' },
  'fitness':           { primary: '#c2410c', bg: '#180d00', surface: '#241400', accent: '#fb923c', ring: '#c2410c40' },
  'crypto':            { primary: '#b45309', bg: '#181500', surface: '#242000', accent: '#fbbf24', ring: '#b4530940' },
  'real estate':       { primary: '#0f766e', bg: '#001816', surface: '#002a28', accent: '#2dd4bf', ring: '#0f766e40' },
  'parenting':         { primary: '#0369a1', bg: '#001018', surface: '#001828', accent: '#38bdf8', ring: '#0369a140' },
  'career':            { primary: '#475569', bg: '#0e1014', surface: '#161c24', accent: '#94a3b8', ring: '#47556940' },
  'mental health':     { primary: '#7e22ce', bg: '#100818', surface: '#1c1030', accent: '#c084fc', ring: '#7e22ce40' },
  'entrepreneurship':  { primary: '#b91c1c', bg: '#180808', surface: '#281010', accent: '#f87171', ring: '#b91c1c40' },
  'trending':          { primary: '#0ea5e9', bg: '#0a1020', surface: '#101830', accent: '#38bdf8', ring: '#0ea5e940' },
};
...
function buildHTML(trend, copy, theme) {
  ...
  const painHTML = copy.painPoints.map(pt => `
    <div style="background:${sf};border:1px solid ${p}33;border-radius:16px;padding:28px;">
      ...
      <p style="color:#cccccc;line-height:1.7;font-size:0.95rem;">${pt.body}</p>
    </div>`).join('');
  ...
  const insideHTML = copy.whatsInside.map(item => `
    <div style="display:flex;gap:16px;align-items:flex-start;padding:20px;background:${sf};border-radius:12px;border:1px solid ${p}22;">
      ...
      <div>
        ...
        <span style="color:#b8b8b8;font-size:0.9rem;">${item.description}</span>
      </div>
    </div>`).join('');

  const forWhoHTML = copy.forWho.map(item => `
    <li style="display:flex;gap:12px;align-items:flex-start;padding:10px 0;border-bottom:1px solid ${p}22;color:#e0e0e0;font-size:0.95rem;">
  ...
  const testimonialsHTML = copy.testimonials.map(t => {
    ...
    <div style="background:${sf};border:1px solid ${p}33;border-radius:16px;padding:28px;">
      ...
      <p style="color:#e8e8e8;line-height:1.7;font-size:0.95rem;margin-bottom:20px;font-style:italic;">"${t.text}"</p>
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div>
          ...
          <span style="color:#b0b0b0;font-size:0.85rem;">${t.role}</span>
        </div>
      </div>
    </div>`;
  }).join('');
  ...
  const faqHTML = copy.faq.map(item => `
    <div style="border-bottom:1px solid ${p}22;">
      ...
      <div class="hidden" style="padding:0 0 20px;color:#cccccc;line-height:1.7;font-size:0.95rem;">${item.a}</div>
    </div>`).join('');

  const trustHTML = copy.trustSignals.map(s => `
    <div style="display:flex;align-items:center;gap:8px;color:#c0c0c0;font-size:0.9rem;">
...
<section style="position:relative;min-height:90vh;display:flex;align-items:center;justify-content:center;text-align:center;padding:80px 24px;overflow:hidden;">
  ...
  <div style="position:relative;z-index:2;max-width:820px;">
    ...
    <p style="font-size:clamp(1rem,2.5vw,1.25rem);color:rgba(255,255,255,0.88);max-width:600px;margin:0 auto 36px;">
      ${copy.subheadline}
    </p>
    ...
    <div style="margin-top:28px;display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:100px;padding:8px 20px;font-size:0.85rem;color:#e0e0e0;">
...
<section style="padding:80px 24px;background:${bg};">
  <div style="max-width:900px;margin:0 auto;">
    ...
    <p style="color:#cccccc;text-align:center;max-width:600px;margin:0 auto 48px;font-size:1rem;">${copy.problemIntro}</p>
...
<section style="padding:80px 24px;background:linear-gradient(180deg,${sf} 0%,${bg} 100%);">
  <div style="max-width:760px;margin:0 auto;text-align:center;">
    ...
    <p style="color:#d4d4d4;font-size:1.05rem;line-height:1.8;margin-bottom:40px;">${copy.solutionBody}</p>
    <div style="background:${p}18;border:2px solid ${p}44;border-radius:16px;padding:24px 32px;display:inline-block;">
      <div style="font-size:2rem;margin-bottom:8px;">📖</div>
      <strong style="color:#fff;font-size:1.1rem;display:block;">${trend.keyword} — Complete Guide</strong>
...
<section style="padding:60px 24px;background:${bg};text-align:center;">
  <div style="max-width:620px;margin:0 auto;background:${sf};border:2px solid ${p}44;border-radius:20px;padding:44px;">
    ...
    <p style="color:#d4d4d4;line-height:1.8;">${copy.guarantee}</p>
  </div>
</section>
...
<section style="padding:80px 24px;background:linear-gradient(135deg,${p}18 0%,${bg} 100%);border-top:1px solid ${p}33;text-align:center;">
  <div style="max-width:660px;margin:0 auto;">
    ...
    <div style="background:${sf};border:1px solid ${p}44;border-radius:16px;padding:32px;margin-bottom:32px;display:inline-block;min-width:280px;">
      <div style="color:#aaaaaa;text-decoration:line-through;font-size:0.9rem;">Regular price: $47</div>
      <div style="font-size:3rem;font-weight:900;color:${ac};line-height:1.1;">$10</div>
      <div style="color:#aaaaaa;font-size:0.85rem;">one-time payment · no subscription</div>
    </div>
    <br/>
    ...
    <div style="margin-top:20px;display:flex;justify-content:center;flex-wrap:wrap;gap:20px;">
      <span style="color:#b0b0b0;font-size:0.85rem;">🔒 Secure Checkout</span>
      <span style="color:#b0b0b0;font-size:0.85rem;">📧 Instant Delivery</span>
      <span style="color:#b0b0b0;font-size:0.85rem;">🛡️ 30-Day Guarantee</span>
    </div>
  </div>
</section>

<!-- ═══ FOOTER ═══ -->
<footer style="background:${sf};border-top:1px solid ${p}22;padding:28px 24px;text-align:center;color:#888888;font-size:0.8rem;">
